...
var word = "bologna"
var lastChar = word.removeAtIndex(word.endIndex.predecessor())
//: __Problem 2__
//:
...
func combineLastCharacters(words:[String]) -> String {
    var result = ""
    for w in words {
        result.append(w[w.endIndex.predecessor()])
    }
    return result
}
...
func isNumber(price: String) -> Bool{
    for n in price.unicodeScalars {
        if !digits.longCharacterIsMember(n.value) {
            return false
        }
    }
    return true
...
var test = MovieArchive()
test.filterByDirector(movies, director: "Christopher Nolan")
