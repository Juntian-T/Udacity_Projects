...
for oldRent in oldMountainViewRent {
    newMountainViewRent.append(oldRent * 1.2)
}
//: ### Exercise 3
...
for (food, like) in polarizingFoods {
    if like {
        print(food + ", yum!")
    } else {
        print(food + ", yuck!")
...
for (name, number) in rapperPhoneNumbers {
    number.stringByReplacingOccurrencesOfString("(415)", withString: "(510)")
}
//: ## Switch Statements
